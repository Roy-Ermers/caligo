...
internal class PaperRenderer : Prowl.Quill.ICanvasRenderer
{
    ...
    public FontFile MonospaceFont;
    public FontFile IconFont;
    ...
    public PaperRenderer(NativeWindow window)
    {
        ...
        Paper = new Paper(this, window.ClientSize.X, window.ClientSize.Y, new());
        ReadConfig();
    ...
    public void ReadConfig()
    {
        ...
        MonospaceFont = new(monospaceFont.FilePath);
        IconFont = new(iconFont.FilePath);
    ...
    public void SetTextureData(object texture, IntRect bounds, byte[] data)
    {
        if (texture is not Texture2D texture2D)
            throw new ArgumentException("Invalid texture type");
        texture2D.SetRectangle(new(bounds.x, bounds.y, bounds.width, bounds.height), data);
    }
