...
public class ChunkMesher
{
	...
	public ChunkMesher(ResourceTypeStorage<Block> blockStorage, ModuleRepository repository, MaterialBuffer materialBuffer)
	{
	...
	private ChunkMesh GenerateMesh(Chunk chunk)
	{
		...
		for (short i = 0; i < Math.Pow(Chunk.Size, 3); i++)
		{
			...
			for (var direction = (Direction)0; direction <= (Direction)5; direction++)
			{
				...
				{
					var newFace = element.ToRenderData(direction, position, variant.Value.Textures ?? [], _materialBuffer, _blockTextureAtlas);
					if (newFace is null)
						continue;
