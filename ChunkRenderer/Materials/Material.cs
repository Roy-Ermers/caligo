...
namespace WorldGen.ChunkRenderer.Materials;
...
public struct Material : IEquatable<Material>
{
    ...
    private readonly Vector3 EncodeTint()
    {
        ...
        {
            return new Vector3(
                MathF.Floor(Tint.Value.X * 15.0f) / 15.0f,
                MathF.Floor(Tint.Value.Y * 15.0f) / 15.0f,
                MathF.Floor(Tint.Value.Z * 15.0f) / 15.0f
            );
        }
