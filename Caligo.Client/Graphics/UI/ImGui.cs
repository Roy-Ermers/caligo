...
{
    private const string UiSettingsFileName = "UI.ini";
    ...
    public void Initialize()
    {
        ...
        // Enable Docking
        io.ConfigFlags |= ImGuiConfigFlags.DockingEnable | ImGuiConfigFlags.DpiEnableScaleViewports |
    ...
    public int BindTextureArray(Texture2DArray textureArray, int slice, int unit = 1)
    {
        ...
        if (currentTextureArray == textureArray.Handle && currentSlice == slice) return _textureArraySliceTexture;
    ...
    private void SetPerFrameImGuiData(float deltaSeconds)
    {
        ...
        if (io.WantSaveIniSettings) SaveSettings();
    ...
    internal static void MouseScroll(OpenTK.Mathematics.Vector2 offset)
    {
        var io = ImGui.GetIO();
    ...
    private void RenderImDrawData(ImDrawDataPtr drawData)
    {
        ...
        {
            var cmdListPtr = drawData.CmdLists[n];
            unsafe
            {
                // Upload vertex/index buffers

                GL.BufferData(BufferTarget.ArrayBuffer, cmdListPtr.VtxBuffer.Size * sizeof(ImDrawVert),
                    cmdListPtr.VtxBuffer.Data, BufferUsageHint.StreamDraw);
                GL.BufferData(BufferTarget.ElementArrayBuffer, cmdListPtr.IdxBuffer.Size * sizeof(ushort),
                    cmdListPtr.IdxBuffer.Data, BufferUsageHint.StreamDraw);

                for (var cmd_i = 0; cmd_i < cmdListPtr.CmdBuffer.Size; cmd_i++)
                {
                    var cmdPtr = cmdListPtr.CmdBuffer[cmd_i];

                    if (cmdPtr.UserCallback != IntPtr.Zero) throw new InvalidOperationException();
                        ...
                        clipRect.W >= 0.0f)
                    {
                        ...
                        GL.DrawElementsBaseVertex(PrimitiveType.Triangles, (int)cmdPtr.ElemCount,
                            DrawElementsType.UnsignedShort, (nint)(cmdPtr.IdxOffset * sizeof(ushort)),
