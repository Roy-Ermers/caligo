...
internal record struct CurrentSelect(
    string Id,
    string[] Options,
    string? Value,
    Rect Rect,
...
{
    private static CurrentSelect? CurrentSelect;
    ...
    {
        var inputBox = InputBox(id, icon, MouseCursor.PointingHand);
        ...
        using (inputBox)
        {
            ...
            Icon(PaperIcon.ArrowDropDown, 16)
                .IsNotInteractable()
        ...
        inputBox.OnPress(e =>
        {
            if (CurrentSelect?.Id == id && CurrentSelect?.Visible == true)
                ...
                CurrentSelect = new CurrentSelect(
                    id,
                    options,
                    currentValue,
                    Rect.Empty,
    ...
    public static void RenderSelectDropdown()
    {
        ...
        foreach (var option in data.Options)
            ListItem(option, selectedValue == option, e => { CurrentSelect = data with { Value = option }; });
    }
}