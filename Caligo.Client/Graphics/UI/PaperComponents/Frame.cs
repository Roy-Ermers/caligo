    ...
    public static ElementBuilder Frame(string name, bool renderHeader = true, char? icon = null)
    {
        ...
        if (renderHeader)
        {
            using var header = Paper.Row(name + "header")
                ...
                .Height(24)
                .Enter();
            if (icon is not null) Icon(icon.Value).PositionType(PositionType.SelfDirected).FontSize(20);

            Text(name).Alignment(TextAlignment.MiddleCenter).Height(UnitValue.StretchOne);
        }
