...
public class ForestWorldGenerator : IWorldGenerator
{
    ...
    public void GenerateChunk(ref Chunk chunk)
    {
        ...
        foreach (var position in new CubeIterator(chunk))
        {
            ...
            if (position.Y < height) chunk.Set(position.ChunkLocalPosition, TerrainBlock);
            ...
            foreach (var feature in features)
            {
                ...
                if (blockId != 0) chunk.Set(position.ChunkLocalPosition, blockId);
            }

            if (position.Y != height + 1 || chunk.Get(position.ChunkLocalPosition) != 0 ||
                pathDistance < 0.025f) continue;
            ...
            chunk.Set(position.ChunkLocalPosition,
                decoration[(int)(Easings.EaseInOutQuad(decorationIndex / 2f + 0.5f) * decoration.Length)]);
        }
    }
