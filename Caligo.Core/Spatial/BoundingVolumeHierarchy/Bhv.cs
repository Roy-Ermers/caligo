...
public sealed class Bhv<T> : IDisposable, IEnumerable<T> where T : class, IBvhItem
{
    ...
    public bool Remove(T item)
    {
        ...
        try
        {
            ...
            if (_root != null && _root.IsLeaf && _root.ItemCount == 0) _root = null;
    ...
    public void Query(BoundingBox queryBox, List<T> results)
    {
        ...
        try
        {
            if (_root != null) QueryRecursive(_root, queryBox, results);
        }
        finally
        {
            _lock.ExitReadLock();
    ...
    public void QueryPoint(WorldPosition point, List<T> results)
    {
        ...
        try
        {
            if (_root != null) QueryPointRecursive(_root, point, results);
        }
        finally
        {
            _lock.ExitReadLock();
    ...
    public T? FindClosest(WorldPosition point, int maxDistance = int.MaxValue)
    {
        ...
        try
        {
            ...
            T? closest = null;
            var closestDistanceSquared = (long)maxDistance * maxDistance;
    ...
    private IEnumerable<T> GetAllItems()
    {
        ...
        while (stack.Count > 0)
        {
            ...
            if (node.IsLeaf)
            {
                ...
                for (var i = 0; i < node.ItemCount; i++) yield return items[i];
            }
            else
            {
    ...
    {
        if (items.Length <= MaxLeafItems) return BvhNode<T>.CreateLeaf(items);

        var splitPos = FindBestSplit(items);
        var leftCount = Math.Max(1, Math.Min(items.Length - 1, splitPos));
    ...
    private static BvhNode<T>? RemoveRecursive(BvhNode<T> node, T item)
    {
        ...
        if (node.Left!.BoundingBox.Intersects(item.BoundingBox)) newLeft = RemoveRecursive(node.Left, item);

        if (node.Right!.BoundingBox.Intersects(item.BoundingBox)) newRight = RemoveRecursive(node.Right, item);
    ...
    private static int FindBestSplit(Span<T> items)
    {
        ...
        if (bestAxis != 2) SortByAxis(items, bestAxis);
    ...
    private static int GetCentroid(BoundingBox box, int axis)
    {
        return axis switch
        {
    ...
    private static float CalculateSAHCost(Span<T> items, int splitPos)
    {
        ...
        var leftProb = (float)leftSA / parentSA;
        var rightProb = (float)rightSA / parentSA;
    ...
    private static BoundingBox CalculateBoundingBox(Span<T> items)
    {
        ...
        for (var i = 1; i < items.Length; i++) result = BoundingBox.Union(result, items[i].BoundingBox);
        return result;
    }
    ...
    {
        if (AutoRebalance && _operationsSinceRebalance >= RebalanceThreshold) Task.Run(Rebalance);
    }
    ...
    private static void FindClosestRecursive(BvhNode<T> node, WorldPosition point, ref T? closest,
        ref long closestDistanceSquared)
    {
        var minDistance = MinDistanceSquaredToBox(point, node.BoundingBox);
        ...
        if (node.IsLeaf)
        {
            ...
            for (var i = 0; i < node.ItemCount; i++)
            {
                var distance = MinDistanceSquaredToBox(point, items[i].BoundingBox);
        ...
        {
            var leftDist = MinDistanceSquaredToBox(point, node.Left!.BoundingBox);
            var rightDist = MinDistanceSquaredToBox(point, node.Right!.BoundingBox);
    ...
    public IEnumerator<T> GetEnumerator()
    {
        ...
        try
        {
            foreach (var item in GetAllItems()) yield return item;
        }
        finally
        {
            _lock.ExitReadLock();
